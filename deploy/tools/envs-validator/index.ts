...
async function validateEnvs(appEnvs: Record<string, string>) {
  ...
  try {
    ...
    const envsWithJsonConfig = [
      ...
      'NEXT_PUBLIC_AD_CUSTOM_CONFIG_URL', // HEAD 特殊默认值
    ];

    for await (const envName of envsWithJsonConfig) {
      if (appEnvs[envName]) {
        let defaultValue = '[]';
        if (envName === 'NEXT_PUBLIC_AD_CUSTOM_CONFIG_URL') {
          defaultValue = '{ "banners": [] }';
        }
        appEnvs[envName] = await getExternalJsonContent(envName) || defaultValue;
      }
    }
